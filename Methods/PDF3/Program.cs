...
public class Program
{
    static void Main()
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("Level 3 Practice Programs");
    ...
    public static int[,] GeneratePCMResults(int count)
    {
        Random random = new Random();
        int[,] scores = new int[count, 3];
        for (int i = 0; i < count; i++)
        {
            scores[i, 0] = random.Next(0, 101);
            scores[i, 1] = random.Next(0, 101);
            scores[i, 2] = random.Next(0, 101);
        }
        return scores;
    }

    public static double[,] CalculateStudentResult(int[,] scores)
    {
        double[,] results = new double[scores.GetLength(0), 4];
        for (int i = 0; i < scores.GetLength(0); i++)
        {
            ...
            results[i, 3] = total;
            // storing total in column 3 for simplicity
        }
        return results;
    }
    ...
    public static void DisplayScorecard(double[,] results)
    {
        ...
        {
            Console.WriteLine($"{i + 1}\t{results[i, 0]}\t{results[i, 1]}\t{results[i, 2]}\t{results[i, 3]}\t{Math.Round((results[i, 3] / 3.0), 2)}\t{Math.Round((results[i, 3] / 300.0) * 100, 2)}");
        }
    }
    ...
    public static int[,] AddMatrices(int[,] a, int[,] b)
    {
        ...
        for (int i = 0; i < a.GetLength(0); i++)
            for (int j = 0; j < a.GetLength(1); j++) result[i, j] = a[i, j] + b[i, j];
        return result;
    }
    ...
    public static int[,] TransposeMatrix(int[,] matrix)
    {
        ...
        for (int i = 0; i < matrix.GetLength(0); i++)
            for (int j = 0; j < matrix.GetLength(1); j++) result[j, i] = matrix[i, j];
        return result;
    }

    public static int Determinant2x2(int[,] matrix) => matrix[0, 0] * matrix[1, 1] - matrix[0, 1] * matrix[1, 0];
